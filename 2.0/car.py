...
class Car:
    ...
    def convertPolygonToMeters(self, pixel):
        return (pixel[0]/3.5,pixel[1]/3.5)
    ...
    def nextPos(self, time):
        ...
        if x == 0:
            b = s * math.sin(math.radians(self.orientation + eps))
            a = s * math.sin(math.radians(90 - (self.orientation + eps)))
        else:
        ...
        if 0 < q <= 90: #1.Quadrant
            ...
            x2 =  self.position[0] + a
            y2 = self.position[0] - b
            
        if self.collision(self.innerPoly, (self.position[0], self.position[0]), (x2, y2)) and self.collision(self.outerPoly, (self.position[0], self.position[0]), (x2, y2)):	
